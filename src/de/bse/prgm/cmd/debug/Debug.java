...
public class Debug implements ICommand {
	 ...
	 * 
	 * @param args
	 *            elements can be string, a variable or a constant. Variables
	 *            and constants can be formatted as ASCII, binary number or hex
	 *            number.
	 */
	...
	public void execute(Machine machine, IConsole console) {
		...
		try {
			for (String arg : args) {
				String tmp = evaluate(arg, machine, console);
				result = tmp.equals("") ? "" : result + tmp;
			}
			console.printLn(result);
		} catch (Exception e) {
			machine.getProgram().addError(new BadFormatRuntimeError("DEBUG"));
	...
	private String evaluate(String string, Machine machine, IConsole console) {
		...
		if (string.startsWith("\"") && string.endsWith("\"")) {
			...
			return createVarInfo(
					string,
					machine.parseIVariable(
							string.replaceAll(
									machine.getAllFormattersAndHashTag(), ""))
							.getValue(), machine);
		}
	}
